...
class ImagePatcher:
            ...
            with rasterio.open(img_path) as src:
                ...
                for y in range(0, padded_img.shape[1] - self.patch_size[1] + 1, y_offset):
                    for x in range(0, padded_img.shape[2] - self.patch_size[0] + 1, x_offset):
                        ...
                        patch_filename = f"test_{img_idx}_{patch_id}.npy"
                        patch_path = os.path.join(SAVE_DIR, patch_filename)
                        imwrite(patch_path, patch)
                        df.loc[len(df)] = [patch_filename, '' , -1]
    ...
    def reconstruct_image(self, json_data_path, patch_shape=(256, 256)):
        ...
        for img_idx in range(len(json_data)):
            ...
            padded_image = np.zeros((padded_h, padded_w, patch_shape[2]))

            # Initialize the mask for counting overlaps
            overlap_mask = np.zeros((padded_h, padded_w, patch_shape[2]))
            ...
            while patch_idx < len(patch_paths):
                ...
                patch = np.load(os.path.join(self.mask_patch_dir, patch_path))
            ...
            np.save(os.path.join(self.reconstruct_save_dir, meta_data['img_filename']), trimmed_image)
